...
class FetchImageSvg extends FetchImage
{
    ...
    public const COLOR_TYPE_STROKE_OUTLINE = FetchImageSvg::STROKE_ATTRIBUTE;
    ...
    public static function createEmptySvg(): FetchImageSvg
    {
    ...
    public function getRequestedOptimizeOrDefault(): bool
    {
        try {
            ...
            return PluginUtility::getConfValue(FetchImageSvg::CONF_SVG_OPTIMIZATION_ENABLE, 1);
        }
    ...
    /**
     * @param $boolean
     * @return FetchImageSvg
     */
    public function setRequestedOptimization($boolean): FetchImageSvg
    {
    ...
    function optimize()
    {
        ...
        if ($this->getRequestedOptimizeOrDefault()) {
             ...
             */
            $confNamespaceToKeeps = PluginUtility::getConfValue(FetchImageSvg::CONF_OPTIMIZATION_NAMESPACES_TO_KEEP);
            ...
            foreach ($this->getXmlDocument()->getDocNamespaces() as $namespacePrefix => $namespaceUri) {
                if (
                    ...
                    && in_array($namespaceUri, FetchImageSvg::EDITOR_NAMESPACE)
                ) {
             ...
             */
            $defaultValues = FetchImageSvg::SVG_DEFAULT_ATTRIBUTES_VALUE;
             ...
             */
            $attributeConfToDelete = PluginUtility::getConfValue(FetchImageSvg::CONF_OPTIMIZATION_ATTRIBUTES_TO_DELETE, "id, style, class, data-name");
            ...
            if (!empty($widthAttributeValue)) {
                ...
                if (!empty($heightAttributeValue)) {
                    ...
                    // ViewBox
                    $viewBoxAttribute = $documentElement->getAttribute(FetchImageSvg::VIEW_BOX);
             ...
             */
            $elementsToDeleteConf = PluginUtility::getConfValue(FetchImageSvg::CONF_OPTIMIZATION_ELEMENTS_TO_DELETE, "script, style, title, desc");
            ...
            $elementsToDeleteIfEmptyConf = PluginUtility::getConfValue(FetchImageSvg::CONF_OPTIMIZATION_ELEMENTS_TO_DELETE_IF_EMPTY, "metadata, defs, g");
            ...
            if (!in_array("svg", $namespaceToKeep)) {
                $documentElement->removeAttributeNS(FetchImageSvg::SVG_NAMESPACE_URI, FetchImageSvg::SVG_NAMESPACE_PREFIX);
            }
    ...
    {
        $viewBox = $this->getXmlDocument()->getXmlDom()->documentElement->getAttribute(FetchImageSvg::VIEW_BOX);
        if ($viewBox !== "") {
    ...
    {
        $viewBox = $this->getXmlDom()->documentElement->getAttribute(FetchImageSvg::VIEW_BOX);
        if ($viewBox !== "") {
            $attributes = $this->getViewBoxAttributes($viewBox);
    ...
    public function processAndGetMarkup(): string
    {
        ...
        if ($viewBox === "") {
            try {
                ...
                LogUtility::error("Svg processing stopped. Bad svg: We can't determine the width of the svg ($this) (The viewBox and the width does not exist) ", FetchImageSvg::CANONICAL);
                return $this->getXmlDocument()->getXmlText();
            }
            try {
                $height = $this->getIntrinsicHeight();
            } catch (ExceptionCompile $e) {
                LogUtility::error("Svg processing stopped. Bad svg: We can't determine the height of the svg ($this) (The viewBox and the height does not exist) ", FetchImageSvg::CANONICAL);
                return $this->getXmlDocument()->getXmlText();
            }
            $this->getXmlDom()->documentElement->setAttribute(FetchImageSvg::VIEW_BOX, "0 0 $width $height");
        }
        ...
        {
            $svgStructureType = FetchImageSvg::ICON_TYPE;
        } else {
            $svgStructureType = FetchImageSvg::ILLUSTRATION_TYPE;
            ...
            if ($svgUsageType === FetchImageSvg::ICON_TYPE || $isInIconDirectory) {
                $svgStructureType = FetchImageSvg::ICON_TYPE;
            }
        ...
        if ($svgUsageType === null) {
            switch ($svgStructureType) {
                case FetchImageSvg::ICON_TYPE:
                    $svgUsageType = FetchImageSvg::ICON_TYPE;
                    break;
                default:
                    $svgUsageType = FetchImageSvg::ILLUSTRATION_TYPE;
                    break;
            }
        }
        switch ($svgUsageType) {
            case FetchImageSvg::ICON_TYPE:
            case FetchImageSvg::TILE_TYPE:
                /**
                 * Dimension
                 *
                ...
                if ($requestedWidth === null) {
                    if ($svgUsageType == FetchImageSvg::ICON_TYPE) {
                        $appliedWidth = FetchImageSvg::DEFAULT_ICON_WIDTH;
                    } else {
                        // tile
                        $appliedWidth = "192";
                    }
            ...
            default:
                ...
                try {
                     ...
                     */
                    $defaultAspectRatio = PluginUtility::getConfValue(FetchImageSvg::CONF_PRESERVE_ASPECT_RATIO_DEFAULT, "xMidYMid slice");
        ...
        switch ($svgStructureType) {
            ...
            case FetchImageSvg::TILE_TYPE:
                 ...
                 */
                $svgColorType = FetchImageSvg::COLOR_TYPE_FILL_SOLID;
                if ($documentElement->hasAttribute(FetchImageSvg::STROKE_ATTRIBUTE)) {
                    $svgColorType = FetchImageSvg::COLOR_TYPE_STROKE_OUTLINE;
                }
                /**
                 * Double color icon ?
                 */
                $isDoubleColor = false;
                if ($svgColorType === FetchImageSvg::COLOR_TYPE_FILL_SOLID) {
                ...
                if (!$isDoubleColor && !$documentElement->hasAttribute("fill")) {
                     ...
                     */
                    $localTagAttributes->addOutputAttributeValue("fill", FetchImageSvg::CURRENT_COLOR);
                ...
                try {
                    ...
                    if ($svgUsageType === FetchImageSvg::ILLUSTRATION_TYPE) {
                ...
                if ($color !== null) {
                    ...
                    switch ($svgColorType) {
                        case FetchImageSvg::COLOR_TYPE_FILL_SOLID:

                            if (!$isDoubleColor) {

                                $localTagAttributes->addOutputAttributeValue("fill", $colorValue);

                                if ($colorValue !== FetchImageSvg::CURRENT_COLOR) {
                                    /**
                                    ...
                                    for ($i = 0; $i < $svgPaths->length; $i++) {
                                        ...
                                        if ($value !== "none") {
                                            if ($nodeElement->parentNode->tagName !== "svg") {
                                                $nodeElement->setAttribute("fill", FetchImageSvg::CURRENT_COLOR);
                                            } else {
                            ...
                            break;
                        case FetchImageSvg::COLOR_TYPE_STROKE_OUTLINE:
                            $localTagAttributes->addOutputAttributeValue("fill", "none");
                            $localTagAttributes->addOutputAttributeValue(FetchImageSvg::STROKE_ATTRIBUTE, $colorValue);
                            ...
                            if ($colorValue !== FetchImageSvg::CURRENT_COLOR) {
                                ...
                                for ($i = 0; $i < $svgPaths->length; $i++) {
                                     ...
                                     */
                                    $nodeElement = $svgPaths[$i];
                                    $value = $nodeElement->getAttribute(FetchImageSvg::STROKE_ATTRIBUTE);
                                    if ($value !== "none") {
                                        $this->getXmlDocument()->removeAttributeValue(FetchImageSvg::STROKE_ATTRIBUTE, $nodeElement);
                                    } else {
        ...
        if ($localTagAttributes->hasComponentAttribute(Dimension::RATIO_ATTRIBUTE)) {
            ...
            $this->getXmlDocument()->setRootAttribute(FetchImageSvg::VIEW_BOX, "0 0 $processedWidth $processedHeight");
        ...
        if ($zoomFactor !== null) {
            ...
            $this->getXmlDocument()->setRootAttribute(FetchImageSvg::VIEW_BOX, "$x $y $processedWidth $processedHeight");
        }
    ...
    public function getFetchPath(): LocalPath
    {
        ...
        try {
            $files[] = ClassUtility::getClassPath(FetchImageSvg::class);
    ...
    public function buildFromUrl(Url $url): FetchImageSvg
    {
        ...
        try {
            $name = $url->getQueryPropertyValue(FetchImageSvg::REQUESTED_NAME_ATTRIBUTE);
            $this->setName($name);
        } catch (ExceptionNotFound $e) {
            // ok
        }
    ...
    public function setPreserveStyle(bool $bool)
    {
     ...
     * @throws ExceptionBadSyntax
     */
    public function setMarkup(string $markup): FetchImageSvg
    {
        $this->xmlDocument = XmlDocument::createXmlDocFromMarkup($markup);
        return $this;
    }

    public function setRequestedType(string $requestedType): FetchImageSvg
    {
