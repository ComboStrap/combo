 ...
 * Image Trait
 *
 * Every instance that uses this trait should implement
 * {@link FetcherImage}. The set method returns it.
 *
 ...
 * width, height, ...)
 *
 * Image may be generated that's why they don't extends from {@link FetcherLocalPath}
 * because they don't depend on a {@link FetcherLocalPath::getOriginalPath() path}
 *
 */
trait FetcherTraitImage
{
     ...
     * @return void
     */
    public function addCommonImagePropertiesToFetchUrl(Url $url = null, string $id = null)
    {
         ...
         */
        if ($requestedWidth !== null || $requestedHeight !== null && $id !== null) {
    ...
    public function buildImagePropertiesFromTagAttributes(TagAttributes $tagAttributes): FetcherImage
    {
        ...
        if ($requestedWidth !== null) {
            try {
                ...
                throw new ExceptionBadArgument("The width value ($requestedWidth) is not a valid integer", FetcherImage::CANONICAL_IMAGE, 0, $e);
            }
        ...
        if ($requestedHeight !== null) {
            try {
                ...
                throw new ExceptionBadArgument("The height value ($requestedHeight) is not a valid integer", FetcherImage::CANONICAL_IMAGE, 0, $e);
            }
        ...
        if ($requestedRatio !== null) {
            try {
                ...
                throw new ExceptionBadArgument("The requested ratio ($requestedRatio) is not a valid value ({$e->getMessage()})", FetcherImage::CANONICAL_IMAGE, 0, $e);
            }
        }
     ...
     * @throws ExceptionCompile|ExceptionBadSyntax - if the svg does not have any view box (TODO: bad)
     */
    ...
    public function getCroppingDimensionsWithRatio(float $targetRatio): array
    {
        ...
        $logicalHeight = FetcherTraitImage::round($logicalWidth / $targetRatio);
        if ($logicalHeight > $this->getIntrinsicHeight()) {
            /**
             * Cropping by height
             */
            $logicalHeight = $this->getIntrinsicHeight();
            $logicalWidth = FetcherTraitImage::round($targetRatio * $logicalHeight);
        }
