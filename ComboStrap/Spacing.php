        ...
        if ($attributes->hasComponentAttribute($spacing)) {
            ...
            if ($logicalTag == \syntax_plugin_combo_cell::TAG) {
                if (StringUtility::startWiths($spacingValue, "m")) {
                    ...
                    LogUtility::msg("We didn't apply the margin value ($spacingValue) on your column because it will make the last column in the grid to go to the line. Apply the following padding instead ($expectedValue)", LogUtility::LVL_MSG_WARNING, GridTag::CANONICAL);
